...
export default function Home() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/artworks`)
      .then(r => r.json())
      .then(data => setTrendingArtworks(Array.isArray(data) ? data : data.artworks || []))
      .catch(() => {})
                ...
                return (
                  <div key={idx} className="group/card cursor-pointer animate-slide-up" style={{ animationDelay: `${idx * 150}ms` }} onClick={() => navigate('/explore')}>
                    ...
                    <div className="flex flex-col items-center text-center px-4 space-y-2">
                      ...
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">artist: <span className="text-white">{typeof art.artist === 'object' ? art.artist?.username : art.artist || 'Unknown Artist'}</span></p>
