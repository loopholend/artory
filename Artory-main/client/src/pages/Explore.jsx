...
export default function Explore() {
  ...
  const fetch_ = async () => {
    ...
    try {
      ...
      let data = Array.isArray(res.data) ? res.data : (res.data.artworks || []);
      
      if (query) {
        data = data.filter(art => 
          art.title?.toLowerCase().includes(query.toLowerCase()) || 
          (typeof art.artist === 'object' ? art.artist?.username?.toLowerCase().includes(query.toLowerCase()) : false)
        );
      }
      
      if (cat !== 'all') {
        data = data.filter(art => art.category?.toLowerCase() === cat.toLowerCase());
      }
                       ...
                       <div className="flex flex-col gap-4 w-full">
                          <div className="flex items-center gap-3">
                             ...
                             <div className="min-w-0">
                                <p className="text-xs font-black text-white truncate leading-none mb-1">{typeof art.artist === 'object' ? art.artist?.username : art.artist}</p>
